        ...
        <div class="col-12">
          <div class="card mb-6">
            ...
            @if (session('success'))
                ...
                <form action="{{ route('editlp.update', $editlp->id) }}" method="POST" enctype="multipart/form-data">
                  @csrf
                    <div class="mb-3 ms-3 me-3">
                        <label for="title" class="form-label">Title</label>
                        <input type="text" name="title" id="title" class="form-control" value="{{$editlp->title ?? old('title')}}" required>
                     </div>
                     <div class="mb-3 ms-3 me-3">
                        <label for="subtitle"class="form-label">Subtitle</label>
                        <input type="text" id="subtitle" name="subtitle" class="form-control" value="{{$editlp->subtitle ?? old('subtitle')}}" required>
                     </div>
                     <div class="mb-3 ms-3 me-3">
                        <label for="description"class="form-label">description</label>
                        <input type="text" id="description" name="description" class="form-control" value="{{$editlp->description ?? old('description')}}" required>
                     </div>
                     <div class="mb-3 ms-3 me-3">
                                <label for="image" class="form-label">Image Example 1</label>
                                <img src="{{ asset('storage/' . $editlp->image) }}" class="img-thumbnail d-block" alt="Image 1" width="150">
                                <input type="file" class="form-control" id="image" name="image">
                            </div>
                     <div class="row ms-3 me-3 justify-content-end">
                        <div class="col-3">
                        <button type="submit" class="btn btn-primary" id="save">Save</button>
                        <a href="{{ route('editlp.index') }}" class="btn btn-danger">Cancel</a>
                    </div>
                    </div>
          </div>
          </div> 
