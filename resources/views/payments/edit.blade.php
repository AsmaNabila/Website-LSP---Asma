            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Edit Payments Data</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Payments / Edit</li>
                        </ol>
                        <!-- <div class="row"> -->
                        <!-- Table -->
          ...
          <div class="card mb-6">
            ...
            <!-- FORM -->
              <div class="table-responsive p-0">
                ...
                <form action="{{route('payments.update', $payments->id)}}" method="POST" id="payments">
                @method('PUT')
                @csrf
                <div class="mb-3 ms-3 me-3">
                    <label for="order_id" class="form-label" required>Order Id</label><br>
                    <select name="order_id" id="order_id" class="form-control" required>
                        @foreach ($idorders as $o)
                            <option value="{{ $o->id }}" @if(old('order_id', $order->id ?? '') == $o->id) selected @endif>{{ $o->id }}</option>
                        @endforeach
                    </select>
                </div>
                     <div class="mb-3 ms-3 me-3">
                        <label type="nama" class="form-label">Payment Date</label>
                        <input type="date" name="payment_date" id="payment_date" class="form-control"aria-label="paymentdate" value="{{$payments->payment_date ?? old('payment_date')}}" required>
                     </div>
                     <div class="mb-3 ms-3 me-3">
                        <label type="payment_method"class="form-label" required>Payment Method</label>
                        <select name="payment_method" class="form-control" id="payment_method" value= required>-- Pilih Payment Method --
                     ...
                     <div class="mb-3 ms-3 me-3">
                        <label type="nama"class="form-label">Amount</label>
                        <input type="number" id="amount" name="amount" class="form-control" aria-label="amount" value="{{$payments->amount ?? old('amount')}}" required>
                     </div>
