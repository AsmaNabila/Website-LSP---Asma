          ...
          <div class="card mb-6">
            ...
            <!-- FORM -->
              <div class="table-responsive p-0">
                ...
                <form action="{{route('orders.update', $orders->id)}}" method="POST" id="orders">
                @method('PUT')
                @csrf
                <div class="mb-3 ms-3 me-3">
                    <label for="customer_id" class="form-label">Customer Id</label><br>
                    <select name="customer_id" id="customer_id" class="form-control" required>
                        @foreach ($idcustomers as $c)
                            <option value="{{ $c->id }}" @if(old('customer_id', $orders->customer_id ?? '') == $c->id) selected @endif>{{ $c->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3 ms-3 me-3">
                    <label for="order_date" class="form-label">Order Date</label>
                    <input type="date" name="order_date" id="order_date" class="form-control" aria-label="date" required value="{{ old('order_date', $order->order_date ?? '') }}">
                </div>
                     <div class="mb-3 ms-3 me-3">
                        <label type="totalamount"class="form-label">Total Amount</label>
                        <input type="totalamount" id="totalamount" name="total_amount" class="form-control" placeholder="masukkan total amount anda" aria-label="totalamount" value="{{$orders->total_amount ?? old('total_amount')}}" required>
                     </div>
                     <div class="mb-3 ms-3 me-3">
                        <label type="status"class="form-label" required>Status</label>
                        <select name="status" class="form-control" id="status" value= required>-- Pilih Status --
                            <option value=""{{$orders->status ?? old('status')}}">"{{$orders->status ?? old('status')}}"</option>
