   ...
   public function index()
    {
        ...
        $dailyEarnings = Orders::whereDate('order_date', $currentDay)->sum('total_amount');
        $weeklyEarnings = Orders::whereBetween('order_date', [$startOfWeek, $endOfWeek])->sum('total_amount');
        $monthlyEarnings = Orders::whereMonth('order_date', $currentMonth)
                                ->whereYear('order_date', $currentYear)->sum('total_amount');
        $annualEarnings = Orders::whereYear('order_date', $currentYear)->sum('total_amount');
        ...
        return view('dashboard.index', compact('dailyEarnings', 'weeklyEarnings', 'monthlyEarnings', 'annualEarnings', 'salesData'));
    }
    /**
